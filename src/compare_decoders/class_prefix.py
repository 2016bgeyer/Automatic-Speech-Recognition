...
class PrefixDecoder:
	...
	def get_most_probable_beams(self, Prob_blank, Prob_not_blank, timestep, beamWidth, beta):
		'''
		...
		if timestep in Prob_blank.prob_table:    
			for prefix in list(Prob_blank.prob_table[timestep].keys()):
				...
				prob_map[prefix] = prob * self.get_num_words(prefix) ** beta
	...
	def __call__(self, network_matrix_output, alphabet, space_token, end_token, blank_token, lm, beamWidth=3, alpha=0.30, beta=5, prune=0.001):
		'''
		...
		# print('prefix Decoder __call__')
		# print(f'network_matrix_output.shape: {network_matrix_output.shape}')
		# print(f'network_matrix_output: {network_matrix_output}')
		# print(f'network_matrix_output.shape: {network_matrix_output.shape}')
		...
		# Initialization
		null_token = ''
		Prob_blank, Prob_not_blank = self.Table(), self.Table()
		...
		for timestep in range(total_timesteps):
				...
				for character_index, character in probable_characters:
					...
					if character == blank_token:
						...
						if prefix and prefix[-1] == character:  # new char is the same as the last char and not empty string
							...
							lm_prob = lm.score(prefix_extended.strip(space_token + end_token)) ** alpha
			...
			prefix_list = self.get_most_probable_beams(Prob_blank, Prob_not_blank, timestep, beamWidth, beta)  # prune all beams to only the beamWidth best beams
